...
export default function TableBookingsPage() {
  ...
  }, [data, scheduleTableId, tableId, temporaryTableId]);
  ...
  const saveTemporaryAvailability = async () => {
    ...
    try {
      ...
      setMessage("Temporary Monday-to-Sunday opening hours saved.");
      await load();
    } catch (error) { setMessage(error instanceof Error ? error.message : "Temporary opening hours could not be saved."); }
    finally { setSaving(false); }
  };
  ...
  const deleteTemporaryAvailability = async (period: { tableId: string; startsOn: string; endsOn: string }) => {
    if (!window.confirm("Remove these temporary opening hours and return to the normal weekly schedule for those dates?")) return;
    setSaving(true);
    try { await request({ action: "delete_temporary_availability", tableId: period.tableId, startsOn: period.startsOn, endsOn: period.endsOn }); setMessage("Temporary opening hours removed."); await load(); }
    catch (error) { setMessage(error instanceof Error ? error.message : "Temporary opening hours could not be removed."); }
    finally { setSaving(false); }
  };
  ...
  return <main className="min-h-screen bg-slate-100 p-4 sm:p-6"><div className="mx-auto max-w-5xl space-y-4"><RequireAuth>
    ...
    {data ? <>
      ...
      {isManageView ? <>
        <details open className="group rounded-2xl border border-sky-300 bg-sky-50 p-5 shadow-sm"><summary className="flex cursor-pointer list-none items-center justify-between gap-3 [&::-webkit-details-marker]:hidden"><div><h2 className="text-2xl font-black text-sky-950">Super User: temporary opening hours</h2><p className="mt-1 text-sm text-sky-800">Enter Monday-to-Sunday hours for a fixed period. Normal hours return automatically afterwards.</p></div><span className="shrink-0 text-sm font-bold text-sky-900"><span className="group-open:hidden">Show</span><span className="hidden group-open:inline">Hide</span></span></summary><div className="mt-4 grid gap-3 sm:grid-cols-3"><label className="text-sm font-medium text-sky-950">Table<select value={temporaryTableId} onChange={(event) => chooseTemporaryTable(event.target.value)} className="mt-1 w-full rounded-lg border border-sky-300 bg-white px-3 py-2">{data.tables.map((table) => <option key={table.id} value={table.id}>{table.name}</option>)}</select></label><label className="text-sm font-medium text-sky-950">Starts<input type="date" value={temporaryStartsOn} onChange={(event) => setTemporaryStartsOn(event.target.value)} className="mt-1 w-full rounded-lg border border-sky-300 bg-white px-3 py-2" /></label><label className="text-sm font-medium text-sky-950">Ends<input type="date" min={temporaryStartsOn} value={temporaryEndsOn} onChange={(event) => setTemporaryEndsOn(event.target.value)} className="mt-1 w-full rounded-lg border border-sky-300 bg-white px-3 py-2" /></label></div><div className="mt-4 overflow-x-auto"><div className="min-w-[620px] space-y-2">{temporaryHours.map((row, index) => <div key={row.weekday} className="grid grid-cols-[130px_120px_1fr_1fr] items-center gap-3 rounded-xl border border-sky-200 bg-white p-3"><strong className="text-slate-950">{days[row.weekday]}</strong><label className="flex items-center gap-2 text-sm font-bold text-slate-700"><input type="checkbox" checked={row.isClosed} onChange={(event) => setTemporaryHours((current) => current.map((entry, entryIndex) => entryIndex === index ? { ...entry, isClosed: event.target.checked } : entry))} />Closed</label><label className="text-xs font-semibold text-slate-600">Opens<input type="time" disabled={row.isClosed} value={row.opensAt} onChange={(event) => setTemporaryHours((current) => current.map((entry, entryIndex) => entryIndex === index ? { ...entry, opensAt: event.target.value } : entry))} className="mt-1 w-full rounded-lg border border-sky-300 px-3 py-2 disabled:opacity-40" /></label><label className="text-xs font-semibold text-slate-600">Closes<input type="time" disabled={row.isClosed} value={row.closesAt} onChange={(event) => setTemporaryHours((current) => current.map((entry, entryIndex) => entryIndex === index ? { ...entry, closesAt: event.target.value } : entry))} className="mt-1 w-full rounded-lg border border-sky-300 px-3 py-2 disabled:opacity-40" /></label></div>)}</div></div><button type="button" disabled={saving || !temporaryTableId || !temporaryStartsOn || !temporaryEndsOn} onClick={() => void saveTemporaryAvailability()} className="mt-4 rounded-lg bg-sky-800 px-4 py-2.5 font-bold text-white disabled:opacity-50">Save temporary hours</button></details>

        <details className="group rounded-2xl border border-emerald-200 bg-emerald-50 p-5 shadow-sm"><summary className="flex cursor-pointer list-none items-center justify-between gap-3 [&::-webkit-details-marker]:hidden"><div><h2 className="text-2xl font-black text-emerald-950">Super User: weekly availability</h2><p className="mt-1 text-sm text-emerald-800">Set the normal bookable hours for each table.</p></div><span className="shrink-0 text-sm font-bold text-emerald-900"><span className="group-open:hidden">Show</span><span className="hidden group-open:inline">Hide</span></span></summary><p className="mt-4 text-sm text-emerald-800">Choose when each pool or snooker table can normally be booked. A closed day has no bookable hours.</p><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-6"><label className="text-sm font-medium text-emerald-950 lg:col-span-2">Table<select value={scheduleTableId} onChange={(event) => setScheduleTableId(event.target.value)} className="mt-1 w-full rounded-lg border border-emerald-300 bg-white px-3 py-2">{data.tables.map((table) => <option key={table.id} value={table.id}>{table.name}</option>)}</select></label><label className="text-sm font-medium text-emerald-950">Day<select value={scheduleWeekday} onChange={(event) => setScheduleWeekday(event.target.value)} className="mt-1 w-full rounded-lg border border-emerald-300 bg-white px-3 py-2">{days.map((dayName, weekday) => <option key={dayName} value={weekday}>{dayName}</option>)}</select></label><label className="flex items-end gap-2 rounded-lg border border-emerald-200 bg-white px-3 py-2 text-sm font-bold text-emerald-950"><input type="checkbox" checked={scheduleEnabled} onChange={(event) => setScheduleEnabled(event.target.checked)} />Bookable</label><label className="text-sm font-medium text-emerald-950">From<input type="time" disabled={!scheduleEnabled} value={scheduleOpens} onChange={(event) => setScheduleOpens(event.target.value)} className="mt-1 w-full rounded-lg border border-emerald-300 bg-white px-3 py-2 disabled:opacity-50" /></label><label className="text-sm font-medium text-emerald-950">Until<input type="time" disabled={!scheduleEnabled} value={scheduleCloses} onChange={(event) => setScheduleCloses(event.target.value)} className="mt-1 w-full rounded-lg border border-emerald-300 bg-white px-3 py-2 disabled:opacity-50" /></label><button type="button" disabled={saving || !scheduleTableId} onClick={() => void saveAvailability()} className="rounded-lg bg-emerald-800 px-4 py-2 font-bold text-white disabled:opacity-50 lg:col-start-6">Save hours</button></div></details>
