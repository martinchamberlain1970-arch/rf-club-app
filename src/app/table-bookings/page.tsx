...
export default function TableBookingsPage() {
  ...
  return <main className="min-h-screen bg-slate-100 p-4 sm:p-6"><div className="mx-auto max-w-5xl space-y-4"><RequireAuth>
    ...
    {data ? <>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="text-2xl font-black text-slate-950">Confirmed bookings</h2><p className="mt-1 text-sm text-slate-600">A chronological list of accepted table bookings.</p><div className="mt-4 divide-y divide-slate-200">{confirmed.length ? confirmed.map((reservation) => <article key={reservation.id} className="flex flex-wrap items-center justify-between gap-3 py-4 first:pt-0 last:pb-0"><div><p className="font-black text-slate-950">{londonDateTime(reservation.starts_at)}–{londonTime(reservation.ends_at)}</p><p className="mt-1 text-slate-700">{tableNames.get(reservation.table_id)} · <strong>{reservationTitle(reservation)}</strong></p><p className="mt-1 text-xs text-slate-500">Booked by {reservation.playerName}</p></div><div className="flex gap-2">{data.isSuper ? <button type="button" disabled={saving} onClick={() => void deleteReservation(reservation.id)} className="rounded-lg border border-red-300 bg-white px-3 py-2 text-sm font-semibold text-red-700">Delete</button> : reservation.booked_for_player_id === data.playerId ? <button type="button" disabled={saving} onClick={() => void cancel(reservation.id)} className="rounded-lg border border-red-300 bg-white px-3 py-2 text-sm font-semibold text-red-700">Cancel</button> : null}</div></article>) : <p className="text-sm text-slate-600">No confirmed upcoming bookings.</p>}</div></section>
