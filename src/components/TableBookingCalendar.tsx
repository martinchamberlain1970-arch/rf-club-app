...
export default function TableBookingCalendar({ tables, reservations, availability, blocks, onChooseSlot }: {
  ...
  return <section className="rounded-2xl border border-emerald-200 bg-white p-5 shadow-sm">
    ...
    <div className="grid grid-cols-7 gap-1">{cells.map((day, index) => {
      ...
      return <button key={key} type="button" disabled={!selectable} onClick={() => setSelectedDate(key)} className={`min-h-20 rounded-lg border p-2 text-left ${colour} ${selectedDate === key ? "border-emerald-800 ring-2 ring-emerald-600" : "border-transparent"} disabled:cursor-default`}><span className="block text-base font-black">{day.getDate()}</span><span className="mt-1 block text-[10px] font-bold sm:text-xs">{closed ? "Closed" : full ? "Full" : limited ? "Limited" : "Available"}</span></button>;
    })}</div>
